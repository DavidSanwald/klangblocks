...
stories.add('a Pad', () => {
  ...
  const timingDefault = 100
  const timing = number(timingLabel, timingDefault)
  return (
    <svg>
      <StyledPad width={width.toString()} height={height.toString()} timing={timing.toString()+"s"}  state={state} onClick={() => null}/>
    </svg>)})
